...
# Resource CSS / Javascript : https://www.w3schools.com
# Last Modification : 06/04/2020
# Version : 1.0 : Creation of this Script
# Version : 1.3 : New Feature
# Version : 1.4 : Fix Bugs + Add Menu
# Version : 1.5 : Add Date
# Version : 1.6 : Add Host Collection
###############################################################
...
if [ "$1" != "" ]; then
    echo "Start the Reporting for the organization :" $1
    if [ "$1" = "all" ]; then
        VARORGA=" "
        VARWEBPAGE="index_ALLORG.html"
    fi
else
    echo "Start the Reporting on all Oraganaizations ? "
    echo "Or cancel and specify an organization :  ./reports_errata.sh MY_ORG"
        while true; do
            read -p "Do you want to start the reporting on all server Y/N ?" yn
            case $yn in
                [Yy]* ) echo "Start the reporting on all Organisation"; VARWEBPAGE="index_ALLORG.html" ; break ;;
                [Nn]* ) hammer organization list  > $VARDESTREP/tempo.lst ; sleep 1 ; cat $VARDESTREP/tempo.lst | grep -v Warning ; exit 0;;
...
echo "<meta name="Simple Errata Web Report" content=\"width=device-width, initial-scale=1\">" >>  $VARDESTREP/$VARWEBPAGE
echo "   <title>SIMPLE ERRATA WEB REPORT</title>" >>  $VARDESTREP/$VARWEBPAGE
...
# TITLE AND MENU :
##################
echo "<h1 align="center">SIMPLE WEB ERRATA REPORT - "$DATE"</h1>" >>  $VARDESTREP/$VARWEBPAGE
echo "<ul>" >>  $VARDESTREP/$VARWEBPAGE
echo "  <li><a href="#" onclick=\"exportTableToCSV('simple_errata_report.csv')\">Export to CSV</a></li>" >>  $VARDESTREP/$VARWEBPAGE
echo "  <li style="float:right"><a class="active" href="#about" TITLE="F.ROFFET">v1.6</a></li>" >>  $VARDESTREP/$VARWEBPAGE
echo "</ul>" >>  $VARDESTREP/$VARWEBPAGE
...
echo "<h2>Filters :</h2>" >>  $VARDESTREP/$VARWEBPAGE
echo "<input type=\"text\" id=\"myInputA\" onkeyup=\"myFunctionA()\" placeholder=\"Search organization ...\"> " >>  $VARDESTREP/$VARWEBPAGE
echo "<input type=\"text\" id=\"myInputB\" onkeyup=\"myFunctionB()\" placeholder=\"Search host ...\"> " >>  $VARDESTREP/$VARWEBPAGE
echo "<input type=\"text\" id=\"myInputC\" onkeyup=\"myFunctionC()\" placeholder=\"Search on group ...\"> " >>  $VARDESTREP/$VARWEBPAGE
echo "<h2> </h2>" >>  $VARDESTREP/$VARWEBPAGE
...
echo "        <th style="cursor:pointer" onclick=\"SortTable(0,'T')\" style=\"width:10%;\" > HOST ID </th>"   >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(1,'T')\" style=\"width:10%;\" > ORGANIZATION </th>"   >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(2,'T')\" style=\"width:10%;\" > HOSTNAME </th>"   >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(3,'T')\" style=\"width:10%;\" > @IP </th>"        >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(4,'T')\" style=\"width:10%;\" > OS NAME </th>"    >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(5,'T')\" style=\"width:10%;\" > OS RELEASE </th>" >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(6,'T')\" style=\"width:10%;\" > PLATFORM  </th>"  >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(7,'T')\" style=\"width:10%;\" > GROUP  </th>"  >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(8,'N')\" style=\"width:6%;\" > Appl. RHSA  </th>" >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(9,'N')\" style=\"width:6%;\" > Appl. RHBA  </th>" >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(10,'N')\" style=\"width:6%;\" > Appl. RHEA  </th>" >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(11,'N')\" style=\"width:6%;\" > Inst. RHSA  </th>" >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(12,'N')\" style=\"width:6%;\" > Inst. RHBA  </th>" >>  $VARDESTREP/$VARWEBPAGE
echo "        <th style="cursor:pointer" onclick=\"SortTable(13,'N')\" style=\"width:6%;\" > Inst. RHEA  </th>" >>  $VARDESTREP/$VARWEBPAGE
...
for id_host in `cat $VARDESTREP/$VARHOSTLIST`
    do
        ...
        HOST_ID=$id_host
        HOST_ORGA=$(hammer host info --id $id_host | grep -m1 Organization | cut -d":" -f 2 | sed 's/ //g')
        ...
        HOST_HGRP=$(hammer host info --id $id_host | grep -A 3 "Host Collections" | grep -m1 Name | cut -d":" -f 2 | sed 's/ //g')
...
printf "<tr>  <td>%s</td>  <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> </tr>\n"  "$HOST_ID" "$HOST_ORGA"  "$HOST_NAME"    "$HOST_IPV4"    "$HOST_OSNA"    "$HOST_OSVE"    "$HOST_VIRT" "$HOST_HGRP" "$APPL_RHSA"    "$APPL_RHBA"    "$APPL_RHEA"    "$INST_RHSA"    "$INST_RHBA"    "$INST_RHEA" "$APPL_ZOOM" "$INST_ZOOM" >>  $VARDESTREP/$VARWEBPAGE

done

echo "<script>                                                  " >>  $VARDESTREP/$VARWEBPAGE

echo "function myFunctionA() {                                  " >>  $VARDESTREP/$VARWEBPAGE
echo "  var input, filter, table, tr, td, i, txtValue;          " >>  $VARDESTREP/$VARWEBPAGE
echo "  input = document.getElementById(\"myInputA\");           " >>  $VARDESTREP/$VARWEBPAGE
...
echo "    td = tr[i].getElementsByTagName(\"td\")[1];           " >>  $VARDESTREP/$VARWEBPAGE
...
echo "function myFunctionB() {                                  " >>  $VARDESTREP/$VARWEBPAGE
echo "  var input, filter, table, tr, td, i, txtValue;          " >>  $VARDESTREP/$VARWEBPAGE
echo "  input = document.getElementById(\"myInputB\");           " >>  $VARDESTREP/$VARWEBPAGE
echo "  filter = input.value.toUpperCase();                     " >>  $VARDESTREP/$VARWEBPAGE
echo "  table = document.getElementById(\"myTable\");           " >>  $VARDESTREP/$VARWEBPAGE
echo "  tr = table.getElementsByTagName(\"tr\");                " >>  $VARDESTREP/$VARWEBPAGE
echo "  for (i = 0; i < tr.length; i++) {                       " >>  $VARDESTREP/$VARWEBPAGE
echo "    td = tr[i].getElementsByTagName(\"td\")[2];           " >>  $VARDESTREP/$VARWEBPAGE
echo "    if (td) {                                             " >>  $VARDESTREP/$VARWEBPAGE
echo "      txtValue = td.textContent || td.innerText;          " >>  $VARDESTREP/$VARWEBPAGE
echo "      if (txtValue.toUpperCase().indexOf(filter) > -1) {  " >>  $VARDESTREP/$VARWEBPAGE
echo "        tr[i].style.display = \"\";                       " >>  $VARDESTREP/$VARWEBPAGE
...
echo "function myFunctionC() {                                  " >>  $VARDESTREP/$VARWEBPAGE
echo "  var input, filter, table, tr, td, i, txtValue;          " >>  $VARDESTREP/$VARWEBPAGE
echo "  input = document.getElementById(\"myInputC\");           " >>  $VARDESTREP/$VARWEBPAGE
echo "  filter = input.value.toUpperCase();                     " >>  $VARDESTREP/$VARWEBPAGE
echo "  table = document.getElementById(\"myTable\");           " >>  $VARDESTREP/$VARWEBPAGE
echo "  tr = table.getElementsByTagName(\"tr\");                " >>  $VARDESTREP/$VARWEBPAGE
echo "  for (i = 0; i < tr.length; i++) {                       " >>  $VARDESTREP/$VARWEBPAGE
echo "    td = tr[i].getElementsByTagName(\"td\")[7];           " >>  $VARDESTREP/$VARWEBPAGE
echo "    if (td) {                                             " >>  $VARDESTREP/$VARWEBPAGE
echo "      txtValue = td.textContent || td.innerText;          " >>  $VARDESTREP/$VARWEBPAGE
echo "      if (txtValue.toUpperCase().indexOf(filter) > -1) {  " >>  $VARDESTREP/$VARWEBPAGE
echo "        tr[i].style.display = \"\";                       " >>  $VARDESTREP/$VARWEBPAGE

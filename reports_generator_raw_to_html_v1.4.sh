...
#        for LINEINRAW in `cat $RAWREPORT`
#            do
#                ID=$(echo $LINEINRAW | cut -d "|" -f 1)
#                ERRATA_ID=$(echo $LINEINRAW | cut -d "|" -f 2)
#                TYPE=$(echo $LINEINRAW | cut -d "|" -f 3)
#                TITLE=$(echo $LINEINRAW | cut -d "|" -f 4)
#                CVE=$(echo $LINEINRAW | cut -d "|" -f 5)
#                printf "<tr> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td></tr>\n" "$ID" "$ERRATA_ID" "$TYPE" "$TITLE" "$CVE" >> $VARDESTREP/$OUTPUT_HTML".html"
#
#            done

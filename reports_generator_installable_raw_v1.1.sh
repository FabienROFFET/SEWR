###################################################
# Description : Create Installable Report for Satellite 6.5
...
for host in `cat $VARDESTREP/$VARHOSTLIST`
    do
        hammer erratum list --host $host --errata-restrict-applicable=false --errata-restrict-installable=true | grep RHSA | while read searchcve; do
